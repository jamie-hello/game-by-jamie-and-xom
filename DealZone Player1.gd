...
func playfirstturn():
	for x in 3: #player 1 plays three tiles to test
		var randomtile = randi_range(0,tileholder.size()-1)
		var chosentile = tileholder.pop_at(randomtile)
		get_parent().get_parent().player1hand.pop_at(randomtile)
...
func add_a_tile(tile):
	...
	tile.global_position = get_children()[get_parent().get_parent().player1hand.size()].global_position
	tile.rest_point = tile.global_position
	tile.drawn_by_player()
	get_parent().get_parent().player1hand.append(tile.Letter) #playerx
	print(get_parent().get_parent().player1hand) #playerx
...
func evaluate():
	...
	"""
	print("player1's letters: ", get_parent().get_parent().player1hand)
