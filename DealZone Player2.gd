...
func playfirstturn():
	for x in 4: #for testing, player2 plays four tiles
		var randomtile = randi_range(0,tileholder.size()-1)
		var chosentile = tileholder.pop_at(randomtile)
		get_parent().get_parent().player2hand.pop_at(randomtile)
...
func add_a_tile(tile):
	...
	tile.global_position = get_children()[get_parent().get_parent().player2hand.size()].global_position #playerx
	tile.rest_point = tile.global_position
	tile.drawn_by_player()
	get_parent().get_parent().player2hand.append(tile.Letter) #playerx
	print(get_parent().get_parent().player2hand) #playerx
...
func evaluate():
	...
	"""
	print("player2's letters: ",get_parent().get_parent().player2hand)
